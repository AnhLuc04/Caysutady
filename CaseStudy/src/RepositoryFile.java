...
//            }
            ...
            File file = new File("C:\\Users\\Admin\\IdeaProjects\\Text2\\CaseStudy\\src\\Shop.txt");
//            file.createNewFile();
            FileWriter os = new FileWriter(file);
            BufferedWriter bff = new BufferedWriter(os);
            while (Name.find() && ID.find() && Price.find()) {
                String line = (
                        ID.group(1).trim().replaceAll(",", "-")
                        + "," + Name.group(1).trim().replaceAll(",",";")
            ...
            bff.close();
            //ĐỌC FILE  TỪ DŨ LIỆU
            FileReader fileReader = new FileReader(new File("C:\\Users\\Admin\\IdeaProjects\\Text2\\CaseStudy\\src\\Shop.txt"));
